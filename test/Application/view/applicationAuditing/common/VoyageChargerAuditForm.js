Ext.define("Soims.view.applicationAuditing.common.VoyageChargerAuditForm", {
    extend: 'Ext.form.Panel',
    alias: 'widget.voyagechargerauditform',
    requires: ['Soims.model.applicationAuditing.AuditType', 'Soims.model.applicationAuditing.ContentType'],
    bodyPadding: 2,
    title: '航次首席科学家意见',

    initComponent: function () {
        var readOnly = !this.isAudit || Soims.component.applicationAuditingCommonVoyageChargerAuditFormVoyageChargerReadOnly;
        this.hidden = Soims.component.applicationAuditingCommonVoyageChargerAuditFormVoyageChargerHidden;
        this.items = [{
            xtype: 'radiogroup',
            fieldLabel: '审核意见',
            labelWidth: 70,
            ...
            listeners: {
                change: function (groupCmp, newValue) {
                    var area = groupCmp.up('voyagechargerauditform').down('textareafield');
        ...
        afterrender: function () {
            ...
            store.load(function () {
                if (store.getCount() != 0) {
                    ...
                    if (Soims.currentUser.currentRoleCode == 'VS') { // 没有审核内容，并且当前角色为航次首席，则form可见
                        me.down('label').hide();
                    } else {
                        me.hide();
                    }
    ...
    initStore: function () {
        var me = this,
            auditState = this.isAudit && Soims.currentUser.currentRoleCode == 'VS' ? Soims.model.applicationAuditing.AuditState.Save.value : Soims.model.applicationAuditing.AuditState.Submit.value,
            store = Ext.create('Soims.store.applicationAuditing.Audit'),
            params = {
                AuditType: Soims.model.applicationAuditing.AuditType.VoyageChargerAudit.value,
