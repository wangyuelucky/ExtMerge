Ext.define("Soims.view.applicationAuditing.common.LegChargerAuditForm", {
    extend: 'Ext.form.Panel',
    alias: 'widget.legchargerauditform',
            ...
            items: [
                ...
                listeners: {
                    change: function (groupCmp, newValue) {
                        var area = groupCmp.up('legchargerauditform').down('textareafield');
        ...
        afterrender: function () {
            ...
            store.load(function () {
                if (store.getCount() != 0) {
                    ...
                    if (Soims.currentUser.currentRoleCode == 'LS') { // 没有审核内容，并且当前角色为航段首席，则form可见
                        me.down('label').hide();
                    } else {
                        me.hide();
                    }
    ...
    initStore: function () {
        var me = this,
            auditState = this.isAudit && Soims.currentUser.currentRoleCode == 'LS' ? Soims.model.applicationAuditing.AuditState.Save.value : Soims.model.applicationAuditing.AuditState.Submit.value,
            store = Ext.create('Soims.store.applicationAuditing.Audit'),
            params = {
                AuditType: Soims.model.applicationAuditing.AuditType.LegChargerAudit.value,
